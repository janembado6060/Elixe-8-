...
public class EventManager implements EventBus {
   ...
   public void subscribe(Listenable listenable) {
      List<Listener> listeners = this.SUBSCRIPTION_CACHE
         .computeIfAbsent(
            listenable,
            o -> Arrays.stream(o.getClass().getDeclaredFields())
   ...
   public void subscribe(Listener listener) {
      List<Listener> listeners = this.SUBSCRIPTION_MAP.computeIfAbsent(listener.getTarget(), target -> new CopyOnWriteArrayList<>());
      int index = 0;

      while (index < listeners.size() && listener.getPriority() <= listeners.get(index).getPriority()) {
         index++;
      }
